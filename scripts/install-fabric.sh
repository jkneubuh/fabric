...
pullBinaries() {
    ...
    download "${BINARY_FILE}" "${RELEASE_ARCHIVE}/fabric/releases/download/v${VERSION}/${BINARY_FILE}"
    if [ $? -eq 22 ]; then
        echo
    ...
    download "${CA_BINARY_FILE}" "${RELEASE_ARCHIVE}/fabric-ca/releases/download/v${CA_VERSION}/${CA_BINARY_FILE}"
    if [ $? -eq 22 ]; then
        echo
